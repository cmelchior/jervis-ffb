...
## Page 35 - The Turnover
The turnover rules specify that two things happen when a Turnover occurs:

1. Any rolls that need to be made as a result of the Turnover occurring are 
   immediately made.
2. The current player's activation ends and that Coach's turn immediately ends.

However, it doesn't mention what happens to optional skills, which means that
in a number of cases it isn't clear exactly how to resolve it.

Some examples:

1. A player is punting the ball out-of-bounds, which triggers a Turnover. When
   it gets thrown back in, can skills be used to catch it?

2. A player is Knocked Down holding the ball. The ball bounces around. Can 
   players use skills to catch it?

For now, Jervis assumes that using skills are at the discretion of the given
coach, but during a turn-over, Jervis seek to end the given player action as
quickly as possible. E.g. you cannot move after a Blitz if a turn-over occured
during the Block.

### Timeout
In BB2205, Timeout is no longer a concept in the rulebook, but for online play
it is still a concept that has benefits.

Jervis treats timeouts as a mix of a turn-over and auto-feeding events to the 
game-model. I.e. If you run out of time during an action, a turn-over is 
triggered and the remaining actions will be auto-generated by the game engine.

This approach gives Jervis the flexibility to adjust the rules around time-outs.
...
## Page 73 - Throw-ins
Bouncing Balls (page 34 in the BB2025 rulebook says that: "...When the ball 
hits the pitch, it will Bounce. When the rules tell you to Bounce the ball..."

The rules for Throw-ins does not say anything about bouncing, neither for or 
against, which would indicate that the ball does indeed not bounce. However, 
this is a change from BB2020, and also the only place when a ball no longer 
bounces: Catch, being Knocked down, kick-off and Punt all bounce.
...
!!! naf "NAF"
    ...
    A ball thrown-in by the crowd Bounces.
