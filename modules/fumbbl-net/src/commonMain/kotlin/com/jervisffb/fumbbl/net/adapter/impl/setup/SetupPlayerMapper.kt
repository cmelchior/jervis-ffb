...
object SetupPlayerMapper: CommandActionMapper {
    ...
    override fun mapServerCommand(
        ...
        when (jervisGame.rules.baseVersion) {
            GameVersion.BB2020 -> {
                newActions.add(PlayerSelected(selectedPlayer), SetupTeamBB2020.SelectPlayerOrEndSetup)
                if (coordinates.x < 0 || coordinates.y > 25) {
                    newActions.add(DogoutSelected, SetupTeamBB2020.PlacePlayer)
                } else {
                    newActions.add(PitchSquareSelected(coordinates.x, coordinates.y), SetupTeamBB2020.PlacePlayer)
                }
            }
            GameVersion.BB2025 -> {
                newActions.add(PlayerSelected(selectedPlayer), SetupTeamBB2025.SelectPlayerOrEndSetup)
                if (coordinates.x < 0 || coordinates.y > 25) {
                    newActions.add(DogoutSelected, SetupTeamBB2025.PlacePlayer)
                } else {
                    newActions.add(PitchSquareSelected(coordinates.x, coordinates.y), SetupTeamBB2025.PlacePlayer)
                }
            }
        }
    }
