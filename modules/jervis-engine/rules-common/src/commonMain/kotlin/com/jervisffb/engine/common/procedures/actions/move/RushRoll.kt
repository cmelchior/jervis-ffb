...
object RushRoll: D6WithRerollProcedure() {
    ...
    override fun onEnterRollProcedure(state: Game, rules: Rules): Command? {
        ...
        if (state.homeTeam.activePrayersToNuffle.contains(PrayerToNuffleTableResult.MOLES_UNDER_THE_PITCH)) {
            modifiers.add(RushModifier.MOLES_UNDER_THE_PITCH_HOME)
        }
        if (state.awayTeam.activePrayersToNuffle.contains(PrayerToNuffleTableResult.MOLES_UNDER_THE_PITCH)) {
