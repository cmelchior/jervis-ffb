...
import com.jervisffb.engine.common.context.DivingCatchContext
 ...
 *
 * See each ruleset's `PassStep.ResolveBounceOrCatch`.
 * See [ThrowIn.ResolveLandOnPitch]
 * See [TheKickOffEvent.ResolveBallLanding]
 * See BB2025 `PuntStep.ResolveLandingOnPitch` (BB2025 only).
 */
object ResolveBallLandingOnPitch: Procedure() {
    ...
    override fun isValid(state: Game, rules: Rules) {
        ...
        if (!ball.coordinates.isOnPitch(rules)) INVALID_GAME_STATE("Ball is not on the pitch: $ball")
    }
