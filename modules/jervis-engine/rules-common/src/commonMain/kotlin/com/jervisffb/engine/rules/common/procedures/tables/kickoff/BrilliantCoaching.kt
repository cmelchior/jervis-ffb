    ...
    object ResolveBrilliantCoaching : ComputationNode() {
        override fun apply(state: Game, rules: Rules): Command {
            ...
            return compositeCommandOf(
                when {
                    kickingResult > receivingResult -> AddTeamReroll(
                        team = state.kickingTeam,
                        reroll = rules.createBrilliantCoachingReroll(state.kickingTeam)
                    )
                    kickingResult < receivingResult -> AddTeamReroll(
                        team = state.receivingTeam,
                        reroll = rules.createBrilliantCoachingReroll(state.receivingTeam)
                    )
