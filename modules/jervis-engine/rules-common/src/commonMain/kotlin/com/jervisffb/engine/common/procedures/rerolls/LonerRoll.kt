...
object LonerRoll: D6WithRerollProcedure() {
    ...
    override fun isValid(state: Game, rules: Rules) = state.assertContext<LonerRollContext>()
    override fun getActionOwner(state: Game): Player = state.getContext<LonerRollContext>().player
    ...
    private fun isSuccess(player: Player, roll: D6Result): Boolean {
        val target = player.getSkill(SkillType.LONER).value as Int
