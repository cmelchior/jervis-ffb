...
fun getResetPlayerTemporaryModifiersCommands(
    ...
    gatherResetPlayerTemporaryModifiersCommands(rules, player, duration, builder)
...
private fun gatherResetPlayerTemporaryModifiersCommands(
    ...
    // Ruleset-specific end-of-turn resets (e.g. BB2025 Shadowing move-usage counter)
    if (duration == Duration.END_OF_TURN) {
        builder.addAll(rules.getEndOfTurnResetCommands(player))
    }
...
fun getResetTeamTemporaryModifiersCommands(
    ...
    gatherResetTeamTemporaryModifiersCommands(rules, activeTeam, duration, builder)
    if (duration != Duration.END_OF_OWN_TEAM_TURN) {
        gatherResetTeamTemporaryModifiersCommands(rules, inactiveTeam, duration, builder)
    }
    return builder.toTypedArray()
}
...
private fun gatherResetTeamTemporaryModifiersCommands(
    ...
    val removableLeaderRerolls = if (
        ...
        team.rerolls
            .filter { rules.isLeaderReroll(it) }
