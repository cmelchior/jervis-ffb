...
object TheKickOffEvent : Procedure(), ChanceObservationHandler {
    ...
    object ResolveKickOffTableEvent : ParentNode() {
        override fun getChildProcedure(state: Game, rules: Rules): Procedure {
            return state.getContext<KickOffEventContext>().result.resolveProcedure(rules)
        }
    ...
    // BB2020 and BB2025 version differs slightly for the touchback, so delegate to
    // the appropriate ruleset via [Rules.kickOffTouchBackNode].
    object SelectTouchBack: ComputationNode() {
        override fun apply(state: Game, rules: Rules): Command {
            return GotoNode(rules.kickOffTouchBackNode)
        }
    }
