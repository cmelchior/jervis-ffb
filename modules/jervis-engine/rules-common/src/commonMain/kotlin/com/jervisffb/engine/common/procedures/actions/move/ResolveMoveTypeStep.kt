...
object ResolveMoveTypeStep : Procedure() {
    ...
    object ResolveMove : ParentNode() {
        override fun getChildProcedure(state: Game, rules: Rules): Procedure {
            return when (state.getContext<MoveContext>().moveType) {
                ...
                MoveType.JUMP -> rules.jumpStep
                MoveType.LEAP -> rules.leapStep
                MoveType.POGO -> rules.pogoStep
            }
        }
    ...
    object SecureTheBall : ParentNode() {
        ...
        override fun getChildProcedure(state: Game, rules: Rules): Procedure = rules.secureTheBallStep
        override fun onExitNode(state: Game, rules: Rules): Command {
            val context = state.getContext<MoveContext>()
