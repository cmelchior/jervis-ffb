...
class Team(
    ...
    val activePrayersToNuffle = mutableSetOf<PrayerToNuffleEvent>()
    ...
    // Plague doctors are a special type of apothecary, so defined in the rules module.
    // Here we just store them as the top-level interface. This means that procedures
    // modifying this must guarantee that only Plague Doctors are added.
    var plagueDoctors = mutableSetOf<Apothecary>()
    ...
    fun hasPrayer(prayer: PrayerToNuffleEvent): Boolean {
