...
object PassStep: Procedure() {
    ...
    object ResolveInaccuratePass: ParentNode() {
        ...
        override fun onExitNode(state: Game, rules: Rules): Command {
            ...
            val ball = state.currentBall()
            val outOfBoundsAt = context.outOfBoundsAt
            return if (outOfBoundsAt != null) {
                compositeCommandOf(
                    SetBallState.Companion.outOfBounds(ball, outOfBoundsAt),
    ...
    object ResolveWildlyInaccuratePass: ParentNode() {
        override fun onEnterNode(state: Game, rules: Rules): Command {
            ...
            val ball = state.currentBall()
            return compositeCommandOf(
                SetBallState.Companion.deviating(ball),
                SetBallLocation(ball, passContext.thrower.coordinates),
                AddContext(DeviateRollContext(passContext.thrower.coordinates))
            )
        }
    ...
    object AttemptPassingInterferenceBeforeGoingOutOfBounds: ParentNode() {
        ...
        override fun onExitNode(state: Game, rules: Rules): Command {
            ...
            return if (!context.continueThrow) {
                compositeCommandOf(
                    UpdateContext(state.getContext<PassContext>().copy(passingInterference = context)),
