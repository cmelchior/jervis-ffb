...
object SetupTeam : Procedure() {
    ...
    override fun onEnterProcedure(state: Game, rules: Rules): Command? {
        ...
        val leaderReroll = team.rerolls.singleOrNull { rules.isLeaderReroll(it) }
    ...
    object SelectPlayerOrEndSetup : ActionNode() {
        ...
        override fun getAvailableActions(state: Game, rules: Rules): List<GameActionDescriptor> {
            ...
            val availablePlayers =
                context.team.filter { player ->
                    val inReserve = (player.location == Dogout && player.state == PlayerDogoutState.RESERVE)
                    val onPitch = (player.location is PitchCoordinate && player.state == PlayerPitchState.STANDING)
                    val hasProblematicStatus = player.statusEffects.any {
                        when (val type = it.type) {
                            ...
                            is PlayerStatusEffectType2020 -> {
                                when (type) {
                                    PlayerStatusEffectType2020.BONE_HEAD -> false
                                    PlayerStatusEffectType2020.REALLY_STUPID -> false
                                    PlayerStatusEffectType2020.UNCHANNELLED_FURY -> false
                                }
                            }
                            else -> INVALID_GAME_STATE("Unsupported type: $type")
                        }
                    }
                    !hasProblematicStatus && (inReserve || onPitch)
                }.let { players ->
            ...
            return when (rules.isSetupValid(state, team).isEmpty()) {
                true -> {
                    ...
                    val trackSecretWeaponCommands = team
                        ...
                        .mapNotNull { player ->
                            rules.setSecretWeaponOnPitchDuringDrive(player, onPitch = true)
                        }
    ...
    fun getAddLeaderRerollCommand(playerAdded: Player): Command? {
        ...
        val reroll = team.rerolls.firstOrNull { rules.isLeaderReroll(it) }
        ...
        return when {
            startOfHalf && !otherLeaderOnPitch -> AddTeamReroll(team,rules.createLeaderTeamReroll(team))
