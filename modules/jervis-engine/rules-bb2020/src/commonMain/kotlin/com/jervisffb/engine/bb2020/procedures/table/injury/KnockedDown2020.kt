 ...
 */
object KnockedDown2020: Procedure() {
    ...
    override fun onEnterProcedure(state: Game, rules: Rules): Command? {
        ...
        return buildCompositeCommand {
            add(SetPlayerIntermediateState(context.player, PlayerIntermediateState.KNOCKED_DOWN))
        ...
        override fun onExitNode(state: Game, rules: Rules): Command {
            ...
            return when {
                isBouncing && context.isPartOfMultipleBlock -> {
                    val mbContext = state.getContext<MultipleBlockContext2020>()
                    compositeCommandOf(
                        SetCurrentBall(null),
                        ExitProcedure()
                    )
