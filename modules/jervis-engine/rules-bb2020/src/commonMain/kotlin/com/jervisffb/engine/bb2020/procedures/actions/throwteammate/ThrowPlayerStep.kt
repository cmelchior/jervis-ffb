...
import com.jervisffb.engine.common.modifiers.LandingModifier
...
import com.jervisffb.engine.common.reports.ReportPlayerLandingOnAnotherPlayer
import com.jervisffb.engine.common.reports.ReportQualityOfThrow
import com.jervisffb.engine.common.reports.ReportStartingThrowTeamMate
import com.jervisffb.engine.common.reports.ReportThrownPlayerGoingOutOfBounds
...
object ThrowPlayerStep: Procedure() {
        ...
        override fun onExitNode(state: Game, rules: Rules): Command {
            ...
            return if (scatterContext.outOfBoundsAt != null) {
                ...
                compositeCommandOf(
                    ...
                    RemoveContext<ScatterRollContext>(),
                    if (state.pitch[landsAt].isOccupied()) {
        ...
        override fun applyAction(action: GameAction, state: Game, rules: Rules): Command {
            return castDiceRoll<D8Result>(action) { bounceRoll ->
                ...
                val landingNode = when {
                    ...
                    target.isOnPitch(rules) && !state.pitch[target].isOccupied() -> ResolveLanding
                    target.isOnPitch(rules) && state.pitch[target].isOccupied() -> ResolveLandingInOccupiedSquare
                ...
                compositeCommandOf(
                    ...
                    UpdateContext(
                        throwContext.copy(
                            target = target,
                            outOfBoundsAt = if (landingNode == ResolveLandingInTheCrowd) thrownPlayer.coordinates else null
                        )
                    ),
    ...
    object ResolveLanding: ParentNode() {
        ...
        override fun onEnterNode(state: Game, rules: Rules): Command {
            ...
            // Add marked modifiers for the pitch
    ...
    object PickupBallAfterLanding: ParentNode() {
        override fun onEnterNode(state: Game, rules: Rules): Command {
            val throwContext = state.getContext<ThrowTeamMateContext>()
            val thrownPlayer = throwContext.thrownPlayer!!
            val ball = state.pitch[thrownPlayer.coordinates].balls.single()
            val pickupContext = PickupRollContext(thrownPlayer, ball)
