...
import com.jervisffb.engine.bb2020.tables.RangeRuler2020
...
abstract class Rules2020(
            ...
            state.activeTeamOrThrow().turnData.let { turnData ->
                ...
                if (turnData.blockActions > 0) {
                    ...
                    val hasEligibleTargets = (player.location as OnPitchLocation)
                        .getSurroundingCoordinates(this@Rules2020, 1)
                        .mapNotNull { state.pitch[it].player }
                ...
                if (turnData.blitzActions > 0) {
                    val hasEligibleBlitzTargets = player.team.otherTeam()
                        .filter { targetPlayer ->  targetPlayer.location.isOnPitch(this@Rules2020) }
                ...
                if (turnData.secureTheBallActions > 0 && !hasUnsteady && !isBigGuy) {
                    ...
                    val eligibleBallExists = state.balls.any { ball ->
                        ...
                        val enemiesInRange = ball.coordinates.getSurroundingCoordinates(
                            rules = this@Rules2020,
                            distance = 2,
                            includeOutOfBounds = false
                        ).any { coordinate ->
                            state.pitch[coordinate].player?.let { p->
                                (p.team != player.team) && this@Rules2020.canMarkPlayers(p)
                            } ?: false
                        }
    ...
    override fun createTeamReroll(team: Team, index: Int): TeamReroll {
        return StandardTeamReroll2020(team.id, index)
    }

    override fun createBrilliantCoachingReroll(team: Team): TeamReroll {
        return BrilliantCoachingReroll2020(team.id)
    }
    ...
    companion object {
        val DEFAULTS = RulesParametersHolder(
            ...
            kickOffEventTable = StandardKickOffEventTable2020,
            prayersToNufflePriceForUnderdog = 50_000,
            prayersToNuffleEnabledForUnderdogDuringPregame = true,
            prayersToNuffleTable = StandardPrayersToNuffleTable2020,
            desperateMeasuresTable = BB7DesperateMeasuresTable,
            weatherTable = StandardWeatherTable2020,
            injuryTable = StandardInjuryTable2020,
            stuntyInjuryTable = StuntyInjuryTable2020,
            casualtyTable = CasualtyTable2020,
            lastingInjuryTable = LastingInjuryTable2020,
            argueTheCallTable = ArgueTheCallTable2020,
            randomDirectionTemplate = RandomDirectionTemplate,
            rangeRuler = RangeRuler2020,
            teamActions = TeamActions2020(),
            rushesPrAction = 2,
            ...
            skillSettings = SkillSettings2020(),
    ...
    @Transient override val fallingOverStep: Procedure = FallingOver2020
    @Transient override val knockedDownStep: Procedure = KnockedDown2020
    @Transient override val kickOffTouchBackNode: Node = TheKickOffEvent2020.TouchBack
...
//    ): Command =
//        state.getContext<MultipleBlockContext2020>().addInjuryReferenceForPlayer(player, injuryContext)
...
) : Rules2020(parameters) {

    companion object {
        val DEFAULTS = Rules2020.DEFAULTS.copy(
 ...
 */
@Serializable(with = BB7RulesSerializer2020::class)
class BB7Rules2020(
    parameters: RulesParametersHolder = DEFAULTS
) : Rules2020(parameters) {
            ...
            inducements = InducementSettings(
                ...
                InducementTypeCommon.entries.forEach { type ->
                    when (type) {
                        ...
                        InducementTypeCommon.EXTRA_TEAM_TRAINING -> getSingle(type).price = 150_000
                        InducementTypeCommon.BRIBE -> { /* Do nothing */ }
                        InducementTypeCommon.WANDERING_APOTHECARY -> { /* Do nothing */ }
                        InducementTypeCommon.MORTUARY_ASSISTANT -> { /* Do nothing */ }
                        InducementTypeCommon.PLAGUE_DOCTOR -> { /* Do nothing */ }
                        InducementTypeCommon.RIOTOUS_ROOKIE -> getSingle(type).enabled = false
                        InducementTypeCommon.HALFLING_MASTER_CHEF -> { /* Do nothing */ }
                        InducementTypeCommon.STANDARD_MERCENARY_PLAYERS -> { /* Do nothing */ }
                        InducementTypeCommon.STAR_PLAYERS -> getInducement(type).enabled = false
                        InducementTypeCommon.INFAMOUS_COACHING_STAFF -> getInducement(type).enabled = false
                        InducementTypeCommon.WIZARD -> getInducement(type).enabled = false
                        InducementTypeCommon.BIASED_REFEREE -> getInducement(type).enabled = false
                        InducementTypeCommon.DESPERATE_MEASURES -> getSingle(type).enabled = true
                    }
                }
                InducementType2020.entries.forEach { type ->
                    when (type) {
                        InducementType2020.BLOODWEISER_KEG -> { /* Do nothing */ }
                        InducementType2020.SPECIAL_PLAY -> { /* Do nothing */ }
                        InducementType2020.WAAAGH_DRUMMER -> getInducement(type).enabled = false
                        InducementType2020.CAVORTING_NURGLINGS -> getInducement(type).enabled = false
                        InducementType2020.DWARFEN_RUNESMITH -> getInducement(type).enabled = false
                        InducementType2020.HALFLING_HOTPOT -> getInducement(type).enabled = false
                        InducementType2020.MASTER_OF_BALLISTICS -> getInducement(type).enabled = false
                        InducementType2020.EXPANDED_MERCENARY_PLAYERS -> { /* Do nothing */ }
                        InducementType2020.GIANT -> getSingle(type).enabled = false
                        InducementType2020.DESPERATE_MEASURES -> {
                            getSingle(type).let {
                                it.enabled = true
                                it.price = 50_000
                                it.max = 5
                        ...
                        InducementType2020.CANOPIC_JAR,
                        InducementType2020.BRETONNIAN_PASTRIES,
                        InducementType2020.BRETONNIAN_DAMSEL ->  { /* Do nothing */ }
                    }
                }
    ...
    // Builder API infrastructure
    override fun toBuilder() = BB7RulesBuilder2020(parameters)
    class BB7RulesBuilder2020(parameters: RulesParameters): RulesParameterBuilder(parameters) {
        override fun build() = BB7Rules2020(buildParameters())
    }
}
...
// abstract [Rules2020] class, which trips a Kotlin 2.4 codegen bug in
...
object BB7RulesSerializer2020 : KSerializer<BB7Rules2020> {
    private val delegate = RulesParametersHolder.serializer()
    override val descriptor: SerialDescriptor =
        SerialDescriptor("com.jervisffb.engine.rules.BB7Rules2020", delegate.descriptor)
    override fun serialize(encoder: Encoder, value: BB7Rules2020) =
        delegate.serialize(encoder, value.parameters)
    override fun deserialize(decoder: Decoder): BB7Rules2020 =
        BB7Rules2020(delegate.deserialize(decoder))
}
