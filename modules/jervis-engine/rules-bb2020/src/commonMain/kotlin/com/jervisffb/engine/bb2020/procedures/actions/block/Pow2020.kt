...
object Pow2020: Procedure() {
    ...
    object ResolveInitialPushSequence: ParentNode() {
        override fun getChildProcedure(state: Game, rules: Rules): Procedure {
            return PushStepInitialMoveSequence2020
        }
        override fun onExitNode(state: Game, rules: Rules): Command {
            ...
            return buildCompositeCommand {
                if (blockContext.isUsingMultiBlock) {
                    val multipleBlockContext = state.getContext<MultipleBlockContext2020>()
                    val property = if (multipleBlockContext.activeDefender == 0) {
                        MultipleBlockContext2020::defender1PushChain
                    } else {
                        MultipleBlockContext2020::defender2PushChain
                    }
     ...
     * instead it is saved in the Injury Pool in [MultipleBlockContext2020] and
    ...
    object DecideToUseStripBall: ComputationNode() {
        override fun apply(state: Game, rules: Rules): Command {
            val isMultipleBlock = state.hasContext<MultipleBlockContext2020>()
    ...
    object ResolveRemainingPushSequenceForSingleBlock: ParentNode() {
        override fun getChildProcedure(state: Game, rules: Rules): Procedure = PushStepResolveSingleBlockPushChain2020
        override fun onExitNode(state: Game, rules: Rules): Command {
