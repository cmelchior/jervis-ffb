...
object RushRoll: D6WithPlayerRerollProcedure() {
    ...
    override fun onEnterRollProcedure(state: Game, rules: Rules): Command? {
        ...
        val team = rushContext.player.team
        if (team.hasFeature(TeamFeatureType.MOLES_UNDER_THE_PITCH)) {
            when (team.isHomeTeam()) {
                true -> modifiers.add(RushModifier.MOLES_UNDER_THE_PITCH_HOME)
                false -> modifiers.add(RushModifier.MOLES_UNDER_THE_PITCH_AWAY)
            }
        }

        return when (modifiers.isNotEmpty()) {
            true -> UpdateContext(rushContext.copyAndAddModifier(*modifiers.toTypedArray()))
            false -> null
        }
    }
