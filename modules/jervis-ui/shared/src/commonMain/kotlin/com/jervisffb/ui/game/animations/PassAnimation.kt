...
class PassAnimation(
    ...
    // (x, y) in pitch coordinate system in pixels
    data class BallState(val x: Float, val y: Float, val scale: Float)

    /**
     * Returns a lamda that can be used to calculate the on-pitch (x, y) position
     * on the pitch as the ball is moving as being thrown.
     *
     ...
     * Scaling Equation while we map between "projectile"-space and "on-pitch"-
     * space.
     *
     ...
     * how the ball scales in "on-pitch"-space.
     *
     * "on-pitch"-space is the 2D coordinate system representing the pitch, i.e.
     * (0, 0) is the top-left corner of the pitch.
     */
    fun getStateCalculator(
        ...
        val onPitchDistancePx = sqrt((endX - startX).toDouble().pow(2) + (endY - startY).toDouble().pow(2))

        // Calculate all parameters for the Projectile motion
        val initialVelocity = calculateVelocity(onPitchDistancePx, angleRadians)
        ...
        // can map back to the pitch coordinate system
        val projectileCoordinateMapper = createCoordinateMapper(startOffset, endOffset)
        val timeToPitchCoordinatesMapper: (Float) -> BallState = { time ->
            ...
            // Convert to "on-pitch"-space
            val (onPitchX, onPitchY) = projectileCoordinateMapper(projectileX)
            BallState(onPitchX.toFloat(), onPitchY.toFloat(), scale.toFloat())
        }
        return Pair(throwDurationSeconds.seconds, timeToPitchCoordinatesMapper)
    }
    ...
    private fun createCoordinateMapper(startOffset: Offset, endOffset: Offset): (Double) -> Pair<Double, Double> {
        ...
        val endX = endOffset.x
        val endY = endOffset.y
        val totalPitchDistancePx = sqrt((endX - startX).toDouble().pow(2) + (endY - startY).toDouble().pow(2))
        return { projectileDistance: Double ->
            val d = projectileDistance / totalPitchDistancePx
